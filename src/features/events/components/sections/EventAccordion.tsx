...
import { EventCollapsibleItem, EventData } from '../../types';

interface CollapsibleAccordionItemProps {
  item: EventCollapsibleItem;
  isOpen: boolean;
  onToggle: () => void;
  index: number;
}
...
const EventAccordion: React.FC<EventAccordionProps> = ({ event }) => {
  ...
  if (!event.collapsibleList || event.collapsibleList.length === 0) return null;
  ...
  return (
    ...
    >
      <div className="overflow-hidden">
        {event.collapsibleList.map((item, index) => (
